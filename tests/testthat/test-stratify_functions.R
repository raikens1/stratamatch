...
test_that("auto_stratify with prog_model + held_frac works", {
  ...
  exp_prog_scores <- predict(exp_prog_model, exp_analysis_set, type = "response")
...
test_that("auto_stratify with prog_model + held_sample works", {
  ...
  a.strat <- auto_stratify(test_dat,
                           ...
                           outcome = "outcome",
                           prog_formula = outcome ~ cont,
                           held_sample = test_dat, # bad practice, but fine for tests
                           size = 4)
  ...
  expect_known_value(a.strat$strata_table, file = "ref_strata_table", update = F)
