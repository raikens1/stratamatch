...
Suppose we wanted to stratify the data manually based on covariates that our experts have selected. Importantly, we can only stratify the data based on categorical variables.  This means that we cannot use `X2` (unless of course we made `X2` categorical by binning).  If we did try and stratify by `X2`, we would get a warning or an error.  Instead, I'm going to make a new variable `X2_pos` which is 1 if `X2` is positive and 0 otherwise.  
...
The other important pieces of info in `m.strat` are the the strata table and the issue table.  The strata table gives the definitions which are used to assign each individual to a stratum:

```{r}
m.strat$strata_table
```

Here we see each of the eight strata that we've divided our data into.  This is a toy example, so each of our strata are very small.
...
This table shows each of our strata and some of the potential issues we may face when trying to match within these strata.  In particular, stratum 6 is 84% control individuals.  This means that there are proportionally many fewer treated individuals in this stratum.
...
Another option is to use `auto_stratify` to automatically create the stratification.  In this process, we specify what model we want to use for the prognostic score and what percent of the control reserve we want to use to do the fitting.  We also need to tell `auto_stratify` which columns of our data set designate the treatment assignment and outcomes.  The final arguement, size, specifies about how large we would like our strata to be.
