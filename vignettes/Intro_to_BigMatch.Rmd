...
After briefly introducing the goals of BigMatch and the approaches it implements on a statistical level, this document will walk through an example of how BigMatch might be used on a toy data set.  We will show how to stratify a data set, how to diagnose the quality of the stratification, and how to match a data set within strata.
...
Once the data set has been stratified, `BigMatch` suggests several diagnostic plots and tables that can be used to assess the size and balance of the strata produced. If the strata are satisfactory, the treatment and control individuals can then be matched.  At this point, the reader may choose to match the data set within each stratum using the `big_match` function, or they may select and implement their own matching scheme. 

# Generating an Example Data Set

We demonstrate the functionality of BigMatch with a toy example.  The `BigMatch` package contains its own function, `make_sample_data`, for just this purpose. 
