...
n_samples <- c(5000, 30000) # For quicker diagnostics
...
time_df <- bind_rows(time_list, .id = "column_label") %>%
  ...
  mutate(process = rep(c("Stratification", "Match by strata in series", "Match without strata", "Match by strata in parallel"), trials)) %>%
...
print("Stratification complete.")
t1 <- proc.time()
mymatch <- big_match_multidplyr(a.strat)
